...
export class ConsignmentIntakeService {
  ...
  async create(
    ...
    this.validateIntakeItems(input.items);

    return this.connection.withTransaction(ctx, async (txCtx) => {
      const repo = this.connection.getRepository(txCtx, ConsignmentIntake);
  ...
  async update(
    ...
    return this.connection.withTransaction(ctx, async (txCtx) => {
      ...
      const updated = await this.findOne(txCtx, intake.id);
      if (!updated) {
  ...
  private validateIntakeItems(items: IntakeItemInput[]): void {
    if (items.length === 0) {
