...
export interface UpdateIntakeInput extends Partial<Omit<CreateIntakeInput, 'storeId'>> {
    id: ID;
}
...
export class ConsignmentIntakeService {
    ...
    async findAll(ctx: RequestContext, storeId: ID): Promise<ConsignmentIntake[]> {
    ...
    async create(ctx: RequestContext, input: CreateIntakeInput): Promise<ConsignmentIntake> {
    ...
    async update(ctx: RequestContext, input: UpdateIntakeInput): Promise<ConsignmentIntake> {
        const repo = this.connection.getRepository(ctx, ConsignmentIntake);
        const itemRepo = this.connection.getRepository(ctx, ConsignmentIntakeItem);
        const quotationRepo = this.connection.getRepository(ctx, ConsignmentQuotation);
        ...
        if (input.deliveryMethod !== undefined) intake.deliveryMethod = input.deliveryMethod ?? null;
        if (input.deliveryTrackingCode !== undefined) intake.deliveryTrackingCode = input.deliveryTrackingCode ?? null;
        if (input.deliveryCost !== undefined) intake.deliveryCost = input.deliveryCost;
