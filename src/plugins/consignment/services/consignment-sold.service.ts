...
export interface UpdateSoldInput extends Partial<Omit<CreateSoldInput, 'storeId'>> {
    id: ID;
}
...
export class ConsignmentSoldService {
    ...
    private aggregateRequestedByQuotation(items: SoldItemInput[]): Map<ID, number> {
    ...
    private async validateItemsBelongToStore(
        ...
        for (const itemInput of items) {
            ...
            if (!quotation) {
                throw new UserInputError(`Quotation ${itemInput.quotationId} not found`);
            }
            if (quotation.storeId !== storeId) {
                throw new UserInputError(`Quotation ${itemInput.quotationId} does not belong to store ${storeId}`);
            }
    ...
    async create(ctx: RequestContext, input: CreateSoldInput): Promise<ConsignmentSold> {
        ...
        const quotationById = await this.validateItemsBelongToStore(ctx, input.storeId, input.items);
    ...
    async update(ctx: RequestContext, input: UpdateSoldInput): Promise<ConsignmentSold> {
        const repo = this.connection.getRepository(ctx, ConsignmentSold);
        const itemRepo = this.connection.getRepository(ctx, ConsignmentSoldItem);
        ...
        if (input.items !== undefined) {
            ...
            const quotationById = await this.validateItemsBelongToStore(ctx, sold.storeId, input.items);
            await this.validateQuantityConstraint(ctx, sold.storeId, input.items, sold.id);
