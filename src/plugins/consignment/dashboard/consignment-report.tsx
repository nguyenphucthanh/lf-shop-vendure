...
export function ConsignmentReportPage() {
    ...
    return (
        <SimplePage title="Consignment Report">
            ...
            {!storeId ? (
                <EmptyState title="Select a store" description="Choose a consignment store to run the report." />
            ) : (
                            ...
                            <TableBody>
                                {rows.map(row => (
                                    <TableRow key={row.quotationId}>
                                        <TableCell>
                                            {row.imageUrl ? <img src={row.imageUrl} alt={row.productName} className="h-12 w-12 rounded object-cover" /> : '—'}
                                        </TableCell>
                                ...
                                {rows.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={12} className="text-center text-sm text-muted-foreground">No report data available.</TableCell>
                                    </TableRow>
                                ) : null}
                            </TableBody>
                        </Table>
