import { api, Button, Card, Input, Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@vendure/dashboard';
...
export function useStores() {
    ...
    useEffect(() => {
        let active = true;
        setLoading(true);
        void api.query(GET_STORES, {}).then(result => {
            if (!active) return;
            const items = result?.customers?.items ?? [];
            setStores(
                items.map((item: any) => ({
                    id: item.id,
                    name: [item.firstName, item.lastName].filter(Boolean).join(' ').trim() || item.emailAddress,
                    emailAddress: item.emailAddress,
                    externalId: item.customFields?.externalId ?? null,
                    defaultDiscountPercent: item.customFields?.defaultDiscountPercent ?? null,
                })),
            );
        }).finally(() => {
...
export function useProductVariants() {
    ...
    useEffect(() => {
        ...
        void api.query(GET_PRODUCT_VARIANTS, {}).then(result => {
            if (!active) return;
            setVariants((result?.productVariants?.items ?? []).map((item: any) => ({
                id: item.id,
                name: item.name,
                sku: item.sku,
...
export function useQuotations(storeId?: string) {
    ...
    useEffect(() => {
        ...
        let active = true;
        setLoading(true);
        void api.query(GET_QUOTATIONS, { storeId }).then(result => {
            if (!active) return;
            setQuotations((result?.consignmentQuotations ?? []) as QuotationOption[]);
        }).finally(() => {
            if (active) setLoading(false);
        });
        return () => {
        ...
        >
            ...
            {props.stores.map(store => (
                <option key={store.id} value={store.id}>
                    {store.name}{store.externalId ? ` (${store.externalId})` : ''}
                </option>
            ))}
        </select>
    );
...
export function LineItemsEditor(props: {
    ...
    const quotationMap = useMemo(() => Object.fromEntries(quotations.map(q => [q.id, q])), [quotations]);
    ...
    function addLine() {
        props.onChange([...props.value, { quotationId: quotations[0]?.id ?? '', quantity: 1 }]);
    }
    ...
    const total = props.value.reduce((sum, line) => {
        ...
        return sum + (quotation?.consignmentPrice ?? 0) * (Number(line.quantity) || 0);
    }, 0);
    ...
    return (
        <Card className="p-4">
            <div className="mb-3 flex items-center justify-between">
                ...
                <Button type="button" variant="secondary" size="sm" onClick={addLine} disabled={!props.storeId || loading}>
                    Add item
                </Button>
            </div>
            <div className="space-y-3">
                {props.value.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Add at least one quotation item.</p>
                ) : null}
                {props.value.map((line, index) => {
                    const quotation = quotationMap[line.quotationId];
                    const subtotal = (quotation?.consignmentPrice ?? 0) * (Number(line.quantity) || 0);
                    return (
                        <div key={index} className="grid grid-cols-12 gap-3 rounded-md border p-3">
                            <div className="col-span-7 space-y-1">
                                ...
                                <select
                                    ...
                                    onChange={event => update(index, { quotationId: event.target.value })}
                                    disabled={loading}
                                >
                            ...
                            <div className="col-span-2 flex flex-col justify-between items-end">
                                <div className="text-sm font-medium">{formatMoney(subtotal)}</div>
                ...
                <div className="flex justify-end text-sm font-medium">Items subtotal: {formatMoney(total)}</div>
            </div>
        </Card>
    );
}

export function KeyValueTable(props: { rows: Array<{ label: string; value: React.ReactNode }> }) {
    return (
        <Table>
            <TableHeader>
                <TableRow>
