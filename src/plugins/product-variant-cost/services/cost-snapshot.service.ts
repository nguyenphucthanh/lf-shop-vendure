...
export class CostSnapshotService implements OnApplicationBootstrap {
  ...
  onApplicationBootstrap(): void {
    this.eventBus.ofType(OrderPlacedEvent).subscribe((event) => {
      void this.snapshotOrderLineCosts(event.ctx, event.order.id).catch(
        (error) => {
          this.logger.error(
  ...
  private async snapshotOrderLineCosts(
    ctx: RequestContext,
    orderId: ID,
  ): Promise<void> {
    ...
    const linesToSnapshot = order.lines.filter(
      (line) => !hasCostSnapshot(line.customFields),
    );
    ...
    for (const line of linesToSnapshot) {
      ...
      if (!costMap.has(key)) {
        const cost =
          await this.productVariantCostService.resolveCostForOrderLine(ctx, {
