...
export function VariantCostBlock({ context }: { context: PageContextValue }) {
    ...
    const availableCurrencies: string[] = activeChannel?.availableCurrencyCodes ?? [
        activeChannel?.defaultCurrencyCode ?? 'USD',
    ];
    ...
    async function handleSave() {
        ...
        try {
            ...
            const updated = result?.upsertProductVariantCost as (typeof costs)[0] | undefined;
            if (updated) {
            setCosts(prev => {
        ...
        <Card className="p-4 space-y-4">
            ...
            <div className="flex gap-2 items-end">
                ...
                <div className="flex-1">
                    <label className="text-xs text-muted-foreground mb-1 block">Cost (decimal)</label>
                    <Input
                        type="number"
                        min="0"
                        step="0.01"
